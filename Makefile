CC = nvcc
CFLAGS = -std=c++11 -m64 -arch=compute_35 -code=sm_35 --compiler-options=-Wall,-O3,-Wno-unused-function,-Wno-unused-local-typedef,-Wno-unused-private-field,-Wno-uninitialized
...
all: prepare-data optimize forward-test

prepare-data: src/prepareData.cu $(addprefix lib/,$(OBJ))
	@mkdir -p $(BIN)
...
forward-test: src/forwardTest.cu $(addprefix lib/,$(OBJ))
	@mkdir -p $(BIN)
